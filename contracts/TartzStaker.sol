...
/// @title Tartz canonical staking interface
contract TartzStaker is ITartzStaker, Multicall {
    ...
    /// @inheritdoc ITartzStaker
    ITartzFactory public immutable override factory;
    /// @inheritdoc ITartzStaker
    ...
    /// @param _factory the Tartz factory
    /// @param _nonfungiblePositionManager the NFT position manager contract address
    /// @param _maxIncentiveStartLeadTime the max duration of an incentive in seconds
    /// @param _maxIncentiveDuration the max amount of seconds into the future the incentive startTime can be set
    constructor(
        ITartzFactory _factory,
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward) external override {
        require(reward > 0, 'TartzStaker::createIncentive: reward must be positive');
        require(
            block.timestamp <= key.startTime,
            'TartzStaker::createIncentive: start time must be now or in the future'
        );
        require(
            key.startTime - block.timestamp <= maxIncentiveStartLeadTime,
            'TartzStaker::createIncentive: start time too far into future'
        );
        require(key.startTime < key.endTime, 'TartzStaker::createIncentive: start time must be before end time');
        require(
            key.endTime - key.startTime <= maxIncentiveDuration,
            'TartzStaker::createIncentive: incentive duration is too long'
        );
    ...
    function endIncentive(IncentiveKey memory key) external override returns (uint256 refund) {
        require(block.timestamp >= key.endTime, 'TartzStaker::endIncentive: cannot end incentive before end time');
        ...
        require(refund > 0, 'TartzStaker::endIncentive: no refund available');
        require(
            incentive.numberOfStakes == 0,
            'TartzStaker::endIncentive: cannot end incentive while deposits are staked'
        );
    ...
    /// @notice Upon receiving a Tartz ERC721, creates the token deposit setting owner to `from`. Also stakes token
    ...
    function onERC721Received(
        ...
        require(
            ...
            'TartzStaker::onERC721Received: not a univ3 nft'
        );
    ...
    function transferDeposit(uint256 tokenId, address to) external override {
        require(to != address(0), 'TartzStaker::transferDeposit: invalid transfer recipient');
        address owner = deposits[tokenId].owner;
        require(owner == msg.sender, 'TartzStaker::transferDeposit: can only be called by deposit owner');
    ...
    function withdrawToken(
        ...
        require(to != address(this), 'TartzStaker::withdrawToken: cannot withdraw to staker');
        Deposit memory deposit = deposits[tokenId];
        require(deposit.numberOfStakes == 0, 'TartzStaker::withdrawToken: cannot withdraw token while staked');
        require(deposit.owner == msg.sender, 'TartzStaker::withdrawToken: only owner can withdraw token');
    ...
    function stakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        require(deposits[tokenId].owner == msg.sender, 'TartzStaker::stakeToken: only owner can stake token');
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        if (block.timestamp < key.endTime) {
            require(
                ...
                'TartzStaker::unstakeToken: only owner can withdraw token before incentive end time'
            );
        }
        ...
        require(liquidity != 0, 'TartzStaker::unstakeToken: stake does not exist');
        ...
        returns (uint256 reward, uint160 secondsInsideX128)
    {
        ...
        (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity) = stakes(tokenId, incentiveId);
        require(liquidity > 0, 'TartzStaker::getRewardInfo: stake does not exist');
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        require(block.timestamp >= key.startTime, 'TartzStaker::stakeToken: incentive not started');
        require(block.timestamp < key.endTime, 'TartzStaker::stakeToken: incentive ended');

        bytes32 incentiveId = IncentiveId.compute(key);

        require(
            incentives[incentiveId].totalRewardUnclaimed > 0,
            'TartzStaker::stakeToken: non-existent incentive'
        );
        require(
            _stakes[tokenId][incentiveId].liquidityNoOverflow == 0,
            'TartzStaker::stakeToken: token already staked'
        );

        (ITartzPool pool, int24 tickLower, int24 tickUpper, uint128 liquidity) =
            NFTPositionInfo.getPositionInfo(factory, nonfungiblePositionManager, tokenId);

        require(pool == key.pool, 'TartzStaker::stakeToken: token pool is not the incentive pool');
        require(liquidity > 0, 'TartzStaker::stakeToken: cannot stake token with 0 liquidity');
